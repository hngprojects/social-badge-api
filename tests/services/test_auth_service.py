...
@patch("app.services.auth._fetch_google_user_info", new_callable=AsyncMock)
...
async def test_authenticate_with_google_propagates_userinfo_errors(
    ...
    with patch(
        "app.services.auth._fetch_google_user_info",
...
async def test_fetch_google_user_info_rejects_invalid_payloads(
    ...
    with patch(
        ...
        with pytest.raises(GoogleOAuthError, match=expected_message):
            await _fetch_google_user_info("token")
...
async def test_fetch_google_user_info_maps_http_status_error() -> None:
...
async def test_fetch_google_user_info_rejects_non_object_payload() -> None:
    mock_response = MagicMock()
    mock_response.raise_for_status.return_value = None
    mock_response.json.return_value = ["not", "an", "object"]
...
async def test_fetch_google_user_info_maps_transport_error() -> None:
    with patch(
        "app.services.auth.httpx.AsyncClient.get",
        new_callable=AsyncMock,
        side_effect=httpx.ConnectError("network down"),
