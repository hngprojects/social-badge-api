...
CONFIG_DIRS=(
  ...
  # — they pull from Prometheus, Loki, and application API telemetry
...
Prometheus, Loki, and the application API simultaneously. Grouping them under
...
if _wait_for_imds; then
  ...
  SLACK_CHANNEL=$(_ssm_get "/lgtm/slack_channel")
  GRAFANA_PASSWORD=$(_ssm_get "/lgtm/grafana_admin_password")
  DUCKDNS_SUBDOMAIN=$(_ssm_get "/lgtm/duckdns_subdomain")
  APP_SERVER_METRICS_HOST=$(_ssm_get "/lgtm/app_server_metrics_host")
  APP_SERVER_SOURCE_CIDR=$(_ssm_get "/lgtm/app_server_source_cidr")
  APP_SERVER_APP_PORT=$(_ssm_get "/lgtm/app_server_app_port")
  APP_SERVER_NODE_EXPORTER_PORT=$(_ssm_get "/lgtm/app_server_node_exporter_port")
  APP_SERVER_HEALTH_URL=$(_ssm_get "/lgtm/app_server_health_url")
  MONITORING_PUBLIC_IP=$(_ssm_get "/lgtm/monitoring_server_public_ip")
  ALERT_EMAIL_RECIPIENTS=$(_ssm_get "/lgtm/alert_email_recipients")
  SMTP_SMARTHOST=$(_ssm_get "/lgtm/smtp_smarthost")
  SMTP_FROM=$(_ssm_get "/lgtm/smtp_from")
  SMTP_AUTH_USERNAME=$(_ssm_get "/lgtm/smtp_auth_username")
  SMTP_AUTH_PASSWORD=$(_ssm_get "/lgtm/smtp_auth_password")
  SMTP_REQUIRE_TLS=$(_ssm_get "/lgtm/smtp_require_tls")
  [[ -z "$GRAFANA_PASSWORD" ]]  && die "Failed to fetch /lgtm/grafana_admin_password from SSM — check instance IAM role"
  [[ -z "$DUCKDNS_SUBDOMAIN" ]] && die "Failed to fetch /lgtm/duckdns_subdomain from SSM — check instance IAM role"
  [[ -z "$APP_SERVER_METRICS_HOST" ]] && die "Failed to fetch /lgtm/app_server_metrics_host from SSM — check instance IAM role"
  [[ -z "$APP_SERVER_SOURCE_CIDR" ]] && die "Failed to fetch /lgtm/app_server_source_cidr from SSM — check instance IAM role"
  [[ -z "$APP_SERVER_APP_PORT" ]] && die "Failed to fetch /lgtm/app_server_app_port from SSM — check instance IAM role"
  [[ -z "$APP_SERVER_NODE_EXPORTER_PORT" ]] && die "Failed to fetch /lgtm/app_server_node_exporter_port from SSM — check instance IAM role"
  [[ -z "$APP_SERVER_HEALTH_URL" ]] && die "Failed to fetch /lgtm/app_server_health_url from SSM — check instance IAM role"
  [[ -z "$MONITORING_PUBLIC_IP" ]] && die "Failed to fetch /lgtm/monitoring_server_public_ip from SSM — check instance IAM role"
  [[ -z "$ALERT_EMAIL_RECIPIENTS" ]] && die "Failed to fetch /lgtm/alert_email_recipients from SSM — check instance IAM role"
  [[ -z "$SMTP_SMARTHOST" ]] && die "Failed to fetch /lgtm/smtp_smarthost from SSM — check instance IAM role"
  [[ -z "$SMTP_FROM" ]] && die "Failed to fetch /lgtm/smtp_from from SSM — check instance IAM role"
  [[ -z "$SMTP_AUTH_USERNAME" ]] && die "Failed to fetch /lgtm/smtp_auth_username from SSM — check instance IAM role"
  [[ -z "$SMTP_AUTH_PASSWORD" ]] && die "Failed to fetch /lgtm/smtp_auth_password from SSM — check instance IAM role"
  [[ -z "$SMTP_REQUIRE_TLS" ]] && SMTP_REQUIRE_TLS=true
  [[ -z "$SLACK_CHANNEL" ]] && SLACK_CHANNEL="#social-badge-devops-alerts"
  ok "Secrets and config fetched from SSM (app target: ${APP_SERVER_METRICS_HOST}:${APP_SERVER_APP_PORT})"
else
  ...
  [[ -z "${DUCKDNS_SUBDOMAIN:-}" ]] && die "DUCKDNS_SUBDOMAIN not set and SSM not available"
  [[ -z "${APP_SERVER_METRICS_HOST:-}" ]] && die "APP_SERVER_METRICS_HOST not set and SSM not available"
  [[ -z "${APP_SERVER_SOURCE_CIDR:-}" ]] && die "APP_SERVER_SOURCE_CIDR not set and SSM not available"
  [[ -z "${APP_SERVER_APP_PORT:-}" ]] && die "APP_SERVER_APP_PORT not set and SSM not available"
  [[ -z "${APP_SERVER_NODE_EXPORTER_PORT:-}" ]] && die "APP_SERVER_NODE_EXPORTER_PORT not set and SSM not available"
  [[ -z "${APP_SERVER_HEALTH_URL:-}" ]] && die "APP_SERVER_HEALTH_URL not set and SSM not available"
  [[ -z "${ALERT_EMAIL_RECIPIENTS:-}" ]] && die "ALERT_EMAIL_RECIPIENTS not set and SSM not available"
  [[ -z "${SMTP_SMARTHOST:-}" ]] && die "SMTP_SMARTHOST not set and SSM not available"
  [[ -z "${SMTP_FROM:-}" ]] && die "SMTP_FROM not set and SSM not available"
  [[ -z "${SMTP_AUTH_USERNAME:-}" ]] && die "SMTP_AUTH_USERNAME not set and SSM not available"
  [[ -z "${SMTP_AUTH_PASSWORD:-}" ]] && die "SMTP_AUTH_PASSWORD not set and SSM not available"
  SMTP_REQUIRE_TLS="${SMTP_REQUIRE_TLS:-true}"
  SLACK_WEBHOOK_URL="${SLACK_WEBHOOK_URL:-}"
  SLACK_CHANNEL="${SLACK_CHANNEL:-#social-badge-devops-alerts}"
  ok "Config loaded from environment (app target: ${APP_SERVER_METRICS_HOST}:${APP_SERVER_APP_PORT})"
fi

MONITORING_DOMAIN="${DUCKDNS_SUBDOMAIN}.duckdns.org"
...
scrape_configs:
  ...
  - job_name: "node-exporter"
    scrape_interval: 15s
    static_configs:
      - targets: ["${APP_SERVER_METRICS_HOST}:${APP_SERVER_NODE_EXPORTER_PORT}"]
    relabel_configs:
  ...
  # ── App server application metrics — social-badge-api ───────────────────
  - job_name: "social-badge-api"
    scrape_interval: 15s
    static_configs:
      - targets: ["${APP_SERVER_METRICS_HOST}:${APP_SERVER_APP_PORT}"]
    relabel_configs:
      - source_labels: [__address__]
        target_label:  instance
        regex:         "([^:]+).*"
...
ALERT_EMAIL_TO=$(grep '^ALERT_EMAIL_RECIPIENTS=' /etc/lgtm/secrets | cut -d= -f2-)
SMTP_HOST=$(grep '^SMTP_SMARTHOST=' /etc/lgtm/secrets | cut -d= -f2-)
SMTP_FROM_ADDR=$(grep '^SMTP_FROM=' /etc/lgtm/secrets | cut -d= -f2-)
SMTP_USER=$(grep '^SMTP_AUTH_USERNAME=' /etc/lgtm/secrets | cut -d= -f2-)
SMTP_PASS=$(grep '^SMTP_AUTH_PASSWORD=' /etc/lgtm/secrets | cut -d= -f2-)
SMTP_TLS=$(grep '^SMTP_REQUIRE_TLS=' /etc/lgtm/secrets | cut -d= -f2-)
SLACK_URL=$(grep '^SLACK_WEBHOOK_URL=' /etc/lgtm/secrets | cut -d= -f2-)
SLACK_ALERT_CHANNEL=$(grep '^SLACK_CHANNEL=' /etc/lgtm/secrets | cut -d= -f2-)

[[ -z "$ALERT_EMAIL_TO" ]] && die "ALERT_EMAIL_RECIPIENTS not set in /etc/lgtm/secrets"
[[ -z "$SMTP_HOST" ]] && die "SMTP_SMARTHOST not set in /etc/lgtm/secrets"
[[ -z "$SMTP_FROM_ADDR" ]] && die "SMTP_FROM not set in /etc/lgtm/secrets"
[[ -z "$SMTP_USER" ]] && die "SMTP_AUTH_USERNAME not set in /etc/lgtm/secrets"
[[ -z "$SMTP_PASS" ]] && die "SMTP_AUTH_PASSWORD not set in /etc/lgtm/secrets"
[[ -z "$SMTP_TLS" ]] && SMTP_TLS=true
[[ -n "$SLACK_URL" && "$SLACK_URL" != https://* ]] && die "SLACK_WEBHOOK_URL must start with https:// when set"
[[ -z "$SLACK_ALERT_CHANNEL" ]] && SLACK_ALERT_CHANNEL="#social-badge-devops-alerts"
...
if [[ -n "$SLACK_URL" ]]; then
  cat >> /etc/lgtm/alertmanager/alertmanager.yml << AMEOF
    slack_configs:
      - api_url:       "${SLACK_URL}"
...
AMEOF
  ...
  info "Slack alerts enabled: ${MASKED}"
else
  info "Slack alerts disabled; email alerts enabled for ${ALERT_EMAIL_TO}"
fi
...
info "Writing email alert template..."
write_config /etc/lgtm/alertmanager/templates/email.tmpl root:alertmanager 640 << 'EOF'
...
EOF
sed -i "s|__GRAFANA_ROOT_URL__|http://${MONITORING_DOMAIN}:3000|g" /etc/lgtm/grafana/grafana.ini
...
# Grafana is publicly reachable. Prometheus and Alertmanager are still bound
# to 127.0.0.1 by their unit files unless you deliberately rebind them.
...
if command -v ufw &>/dev/null; then
  ...
  # Public monitoring access through IP/domain.
  ufw allow 80/tcp comment "Monitoring HTTP"
  ufw allow 443/tcp comment "Monitoring HTTPS"
  ...
  # OTel ingestion ports: restricted to the existing app server CIDR.
  ufw allow from "${APP_SERVER_SOURCE_CIDR}" to any port 3100 proto tcp comment "Loki OTLP from app server"
  ufw allow from "${APP_SERVER_SOURCE_CIDR}" to any port 4317 proto tcp comment "Tempo OTLP gRPC from app server"
  ufw allow from "${APP_SERVER_SOURCE_CIDR}" to any port 4318 proto tcp comment "Tempo OTLP HTTP from app server"

  # Enable without interactive prompt
  ufw --force enable
  ok "ufw enabled: SSH, HTTP(:80), HTTPS(:443), Grafana(:3000), Prometheus(:9090), Alertmanager(:9093), Pushgateway(:9091), OTel(:3100/:4317/:4318 from ${APP_SERVER_SOURCE_CIDR}) open"
...
if [[ "$ERRORS" -eq 0 ]]; then
  ...
  echo ""
  echo -e "  App metrics target: ${BLU}${APP_SERVER_METRICS_HOST}:${APP_SERVER_APP_PORT}${RST}"
  echo -e "  App health target:  ${BLU}${APP_SERVER_HEALTH_URL}${RST}"
  echo ""
else
