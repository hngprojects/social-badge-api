...
def hash_token(token: str) -> str:
    """Computes a SHA-256 hash of a raw token string for secure database storage or
    lookup."""
...
async def blacklist_token(redis: Redis, jti: str, remaining_seconds: int) -> None:
    ...
    Ensures that a logged-out or invalidated token cannot be reused before it naturally
    expires.
    """
...
async def is_token_blacklisted(redis: Redis, jti: str) -> bool:
    """Checks whether a JWT JTI identifier is currently registered as blacklisted in
    Redis."""
...
async def get_verified_user_id(
    ...
    """Retrieves and deletes (consumes) user ID associated with a verification token
    hash in Redis.
...
def create_access_token(user_id: UUID) -> str:
    """Creates a JWT access token for a user with configured expiration time and a
    unique JTI."""
...
async def store_password_reset_token(
    ...
    """Stores the password reset token hash mapped to user ID in Redis with a short-
    lived TTL."""
...
async def get_password_reset_user_id(
    ...
    """Retrieves and deletes (consumes) user ID associated with a password reset token
    hash in Redis."""
...
async def store_google_oauth_state(redis: Redis, state: str) -> None:
    """Stores a Google OAuth state parameter in Redis with a short-lived TTL to prevent
    CSRF attacks."""
